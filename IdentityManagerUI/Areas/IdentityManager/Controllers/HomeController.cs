...
namespace IdentityManagerUI.Areas.IdentityManager.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        private static void EnrichClaimTypes(Dictionary<string, string> claimTypes, string connectionString) {
            string claim;
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                SqlCommand CMD = new SqlCommand($"select DISTINCT ClaimType from AspNetUserClaims WHERE LEFT(ClaimType, 7)<>'http://' ORDER BY ClaimType", con);
                CMD.CommandType = CommandType.Text;
                con.Open();
                SqlDataReader reader = CMD.ExecuteReader();
                while (reader.Read())
                {
                    claim = reader.GetString(0);
                    if (!claimTypes.ContainsKey(claim)) {
                        claimTypes.Add(claim, claim);
                    }
                }
                con.Close();
            }
        ...
        public HomeController(UserManager<ApplicationUser> userManager, RoleManager<ApplicationRole> roleManager, ILogger<HomeController> logger, IConfiguration config, IHttpContextAccessor httpContext)
        {
            _userManager = userManager;
            _roleManager = roleManager;
            _logger = logger;
            string hostName = ConfigHelpers.ConfigHttpHelpers.GetHostByHttpContext(config, httpContext);
            _connectionString = ConfigHelpers.ConfigHelpers.GetConnectionString(config, hostName); // config.GetConnectionString("IdentityDB");
            ...
            EnrichClaimTypes(_claimTypes, _connectionString);
        }
        private string GetClaimKey(string key)
        {
            if (_claimTypes.ContainsKey(key))
        ...
        public IActionResult UserList(int draw, List<Dictionary<string, string>> columns, List<Dictionary<string, string>> order, int start, int length, Dictionary<string, string> search)
        {
            ...
            var result = new
            {
                ...
                data = qry.Skip(start).Take(length).ToArray().Select(u => new {
                    ...
                    Claims = u.Claims.Select(c => new KeyValuePair<string, string>(
                        string.IsNullOrEmpty((kvp = _claimTypes.SingleOrDefault(x => x.Value == c.ClaimType)).Key) ? c.ClaimType : kvp.Key
                    , c.ClaimValue)),
        ...
        public async Task<ActionResult> UpdateUser(string id, string email, bool locked, string[] roles, List<KeyValuePair<string, string>> claims)
        {
            try
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    foreach (var kvp in claims.Where(a => !userClaims.Any(b => GetClaimKey(a.Key) == b.Type && a.Value == b.Value))) {
                        await _userManager.AddClaimAsync(user, new Claim(GetClaimKey(kvp.Key), kvp.Value));
                    }

                    foreach (var claim in userClaims.Where(a => !claims.Any(b => a.Type == GetClaimKey(b.Key) && a.Value == b.Value)))
        ...
        public IActionResult RoleList(int draw, List<Dictionary<string, string>> columns, List<Dictionary<string, string>> order, int start, int length, Dictionary<string, string> search)
        {
            ...
            var result = new
            {
                ...
                data = qry.Skip(start).Take(length).ToArray().Select(r => new {
                    ...
                    Name = r.Name,
                    //Key/Value props not camel cased (https://github.com/dotnet/corefx/issues/41309)
                    Claims = r.Claims.Select(c => new KeyValuePair<string, string>(
                        string.IsNullOrEmpty((kvp = _claimTypes.SingleOrDefault(x => x.Value == c.ClaimType)).Key) ? c.ClaimType : kvp.Key
                        , c.ClaimValue))
                })
            };
        ...
        public async Task<ActionResult> UpdateRole(string id, string name, List<KeyValuePair<string, string>> claims)
        {
            try
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    foreach (var kvp in claims.Where(a => !roleClaims.Any(b => GetClaimKey(a.Key) == b.Type && a.Value == b.Value)))
                        await _roleManager.AddClaimAsync(role, new Claim(GetClaimKey(kvp.Key), kvp.Value));

                    foreach (var claim in roleClaims.Where(a => !claims.Any(b => a.Type == GetClaimKey(b.Key) && a.Value == b.Value)))
